 ...
 * @version : SQLRunner.java, v 0.1 2021年12月08日 10:04 上午 zhongping.czp Exp $
 */
public class SQLRunner {
    ...
    private void handleLine(StringBuilder command, String line) throws SQLException {
        ...
        if (lineIsComment(trimmedLine)) {
            ...
            command.append(trimmedLine, 0, line.lastIndexOf(delimiter));
    ...
    private boolean commandReadyToExecute(String trimmedLine) {
        if (fullLineDelimiter && trimmedLine.equals(delimiter)) { return true; }
        // issue #561 remove anything after the delimiter
        // `channel` varchar(16) DEFAULT NULL COMMENT 'alipay;taobao;xx' will error
        boolean isEndSemicolon = false;
        int len = trimmedLine.length();
        int index = trimmedLine.lastIndexOf(';');
        if (index > -1 && len == index + 1) {
            isEndSemicolon = true;
            if (len > 1 && trimmedLine.charAt(len - 2) != ')' && trimmedLine.startsWith("insert")) {
                char c;
                int count = 0;
                //process:insert into tmp_name(desc)value('xx,yy;\nxx')
                for (int i = len - 1; i > -1; i--) {
                    c = trimmedLine.charAt(i);
                    if (c == '(') { count++; } else if (c == ')') { count--; }
                }
                isEndSemicolon = count == 0;
            }
        }
        // return !fullLineDelimiter && trimmedLine.contains(delimiter) || fullLineDelimiter && trimmedLine.equals(delimiter);
        return !fullLineDelimiter && isEndSemicolon;
    }
